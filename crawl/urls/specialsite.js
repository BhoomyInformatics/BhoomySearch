...
class Server {
    constructor() {
        ...
        this.crawlList = [];
        this.parPage = 10; // Number of sites to crawl per page
        this.specialUrls = [];

        this.crawler = new SearchEngineCrawler(con, {
            maxDepth: 3,
            maxPagesPerDomain: 500,   // Your setting: 250 pages per domain
            batchSize: 10,            // Your setting: 10 URLs concurrent per depth
            maxPages: 500,            // Your setting: 250 pages max queue size
            batchDelay: 2000,
            depthDelay: 3000,
            respectRobots: true,
            timeout: 30000
    ...
    async loadSiteUrls() {
        try {
            const data = await fs.readFile('splsiteurl.txt', 'utf-8');
            ...
            console.log(`Loaded ${urls.length} URLs from splsiteurl.txt`);
    ...
    async updateSpecialSites() {
        try {
            console.log('Updating special sites status...');
            ...
            // Update all special sites to active and unlocked
            ...
            console.log(`Updated ${updateResult.affectedRows} special sites`);

            // Get current status of all special sites
            ...
            if (statusResult.length === 0) {
                ...
                for (const domain of this.specialUrls) {
                    try {
            ...
            console.error('Error updating special sites:', error);
            return 0;
        }
    }
    ...
    async loadSiteCount() {
        try {
            ...
            // Load special URLs first
            this.specialUrls = await this.loadSiteUrls();
            
            if (this.specialUrls.length === 0) {
                console.log('No URLs found in splsiteurl.txt');
                this.sitesCount = 0;
                return;
            }

            // Update all special sites to active status
    ...
    async crawlerStart() {
        try {
            console.log('Starting special sites crawler...');
            
            if (!this.sitesCount) await this.loadSiteCount();

            if (this.sitesCount === 0) {
                console.log('No special sites to crawl at this time');
                return;
            }
...
// Schedule job to run every 2 hours (news sites need more frequent updates)
