...
// Function to process URLs from domain.txt and categorize them
async function processUrls() {
    const fileStream = fs.createReadStream('domain.txt');
    ...
    for await (let url of rl) {
        ...
        if (domain && await validateDomain(domain)) {
            ...
            if (result.status === 'active') {
                appendToFile('activeurls.csv', url);
            } else {
                appendToFile('inactive.txt', url);
            }
        } else {
            appendToFile('inactive.txt', url);
        }
