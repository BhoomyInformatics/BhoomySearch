...
// Function to process URLs from domain3.txt and categorize them
async function processUrls() {
    const fileStream = fs.createReadStream('domain3.txt');
    ...
    for await (let url of rl) {
        ...
        if (domain && await validateDomain(domain)) {
            ...
            if (result.status === 'active') {
                appendToFile('activeurls3.csv', url);
            } else {
                appendToFile('inactive3.txt', url);
            }
        } else {
            appendToFile('inactive3.txt', url);
        }
