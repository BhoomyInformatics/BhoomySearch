...
// Function to process URLs from domain4.txt and categorize them
async function processUrls() {
    const fileStream = fs.createReadStream('domain4.txt');
    ...
    for await (let url of rl) {
        ...
        if (domain && await validateDomain(domain)) {
            ...
            if (result.status === 'active') {
                appendToFile('activeurls4.csv', url);
            } else {
                appendToFile('inactive4.txt', url);
            }
        } else {
            appendToFile('inactive4.txt', url);
        }
