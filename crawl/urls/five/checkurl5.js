...
// Function to process URLs from domain5.txt and categorize them
async function processUrls() {
    const fileStream = fs.createReadStream('domain5.txt');
    ...
    for await (let url of rl) {
        ...
        if (domain && await validateDomain(domain)) {
            ...
            if (result.status === 'active') {
                appendToFile('activeurls5.csv', url);
            } else {
                appendToFile('inactive5.txt', url);
            }
        } else {
            appendToFile('inactive5.txt', url);
        }
