...
// Function to process URLs from domain2.txt and categorize them
async function processUrls() {
    const fileStream = fs.createReadStream('domain2.txt');
    ...
    for await (let url of rl) {
        ...
        if (domain && await validateDomain(domain)) {
            ...
            if (result.status === 'active') {
                appendToFile('activeurls2.csv', url);
            } else {
                appendToFile('inactive2.txt', url);
            }
        } else {
            appendToFile('inactive2.txt', url);
        }
