...
class Server {
    ...
    async loadSiteUrls() {
        try {
            const data = await fs.readFile('specialsite.txt', 'utf-8');
            ...
            logger.info(`Loaded ${urls.length} URLs from specialsite.txt`);
    ...
    async loadSiteCount() {
        try {
            ...
            if (this.specialUrls.length === 0) {
                logger.info('No URLs found in specialsite.txt');
                this.sitesCount = 0;
                return;
            }
