...
class Server {
    ...
    async fetchUnlockedSites() {
        const rows = await db.query(`
            SELECT * FROM sites
            WHERE (sites.site_url LIKE '%.in%' OR sites.site_url LIKE '%.info%')
    ...
    async tryActivateInactiveSites() {
        const activated = await db.query(`
            ...
            WHERE (site_url LIKE '%.in%' OR site_url LIKE '%.info%') AND site_active = 0
        `);
        const affected = (activated && typeof activated.affectedRows === 'number') ? activated.affectedRows : 0;
        if (affected > 0) {
            console.log(`Activated ${affected} .in/.info sites (were inactive). Will retry fetch.`);
            return true;
        }
        return false;
    }
    ...
    async crawlerStart() {
        try {
            console.log('Starting crawler for .in, .info domains...');
