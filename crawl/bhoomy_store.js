// bhoomy_store.js - Crawls .store  .online, and .news domains
...
class Server {
    ...
    async loadSiteCount() {
        try {
            console.log('Loading site count for .store domains...');
            ...
            const result = await con.query(`
                ...
                FROM sites
                WHERE (sites.site_url LIKE '%.store%'
                    OR sites.site_url LIKE '%.online%'
                    OR sites.site_url LIKE '%.news%')
            ...
            console.log(`Found ${this.sitesCount} sites to crawl (.store domains)`);
        } catch (error) {
    ...
    async crawlerStart() {
        try {
            console.log('Starting crawler for .store domains...');
            ...
            while (true) {
                ...
                const rows = await con.query(`
                    ...
                    WHERE (site_url LIKE '%.store%'
                           OR site_url LIKE '%.online%'
                           OR site_url LIKE '%.news%')
                      AND site_active = 1
                      AND site_locked = 0
...
const job = schedule.scheduleJob('0 */12 * * *', async () => {
    console.log(`\n=== Starting scheduled crawl for .store domains at ${new Date().toISOString()} ===`);
    await server.crawlerStart();
    console.log(`=== Completed scheduled crawl for .store domains at ${new Date().toISOString()} ===\n`);
});

console.log('Bhoomy .store crawler started with schedule: every 12 hours');
