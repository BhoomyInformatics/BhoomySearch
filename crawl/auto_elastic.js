...
async function indexData(indexName, query, processData, idField) {
    try {
        ...
        for (let i = 0; i < Math.ceil(rowCount / pageSize); i++) {
            ...
            const rows = await con.query(`SELECT * FROM ${indexName === 'sites1' ? 'site'
                                : indexName === 'sitedata1' ? 'site_data'
                                : indexName === 'images' ? 'site_img'
                                : 'site_videos'} LIMIT ${offset}, ${pageSize}`);
...
async function runAutoSync() {
    while (true) {         		
		const sites1Promise = indexData('sites1', 'SELECT COUNT(*) as c FROM site', processAndIndexData, 'site_id');
        const sitedata1Promise = indexData('sitedata1', 'SELECT COUNT(*) as c FROM site_data', processAndIndexData, 'site_data_id');
        const imagesPromise = indexData('images', 'SELECT COUNT(*) as c FROM site_img', processAndIndexData, 'site_img_id');
        const videosPromise = indexData('videos', 'SELECT COUNT(*) as c FROM site_videos', processAndIndexData, 'site_videos_id');

        await Promise.all([sites1Promise, sitedata1Promise, imagesPromise, videosPromise]);
