// bhoomy_com.js - Crawls .com domains
...
class Server {
    constructor() {
        this.sites;
        this.page = 0;
        // New batching strategy: fetch 200 sites from DB at a time, then process in sub-batches of 20
        this.fetchBatchSize = 200;   // Number of sites fetched from DB per window
        this.subBatchSize = 20;      // Number of sites processed per sub-batch
        this.pageNo = 0;
    ...
    async loadSiteCount() {
        try {
            console.log('Loading site count for .com domains...');
            ...
            console.log(`Found ${this.sitesCount} sites to crawl (.com domains)`);
        } catch (error) {
    ...
    async crawlerStart() {
        try {
            console.log('Starting crawler for .com domains...');
            ...
            let batchNumber = 0;
            // Continuous windowed processing: fetch 200 at a time, process in sub-batches of 20
            while (true) {
                batchNumber++;
                // Fetch a window of unlocked .com sites
                const rows = await con.query(`
                    SELECT site_id, site_url FROM sites
                    WHERE site_url LIKE '%.com%'
                      AND site_active = 1
                      AND site_locked = 0
                ...
                // Lock fetched sites for this script to allow parallel runners safely
                ...
                // Read back the locked rows for this script to ensure we only process what we locked
                ...
                // Process in sub-batches of 20 to control resource usage
                ...
                // Safety: ensure all are unlocked after this window
...
const job = schedule.scheduleJob('0 */12 * * *', async () => {
    console.log(`\n=== Starting scheduled crawl for .com domains at ${new Date().toISOString()} ===`);
    await server.crawlerStart();
    console.log(`=== Completed scheduled crawl for .com domains at ${new Date().toISOString()} ===\n`);
});

console.log('Bhoomy .com crawler started with schedule: every 12 hours');
