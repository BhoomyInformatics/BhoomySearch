...
class Server {
    ...
    /** Fetch next chunk of unlocked .com sites (max FETCH_SIZE). Returns [] when none. */
    ...
    /** If no sites found, try activating inactive .com sites and return true so caller can retry once. */
    async tryActivateInactiveSites() {
        const activated = await db.query(`
            UPDATE sites SET site_active = 1
            WHERE site_url LIKE '%.com%' AND site_active = 0
        `);
        const affected = (activated && typeof activated.affectedRows === 'number') ? activated.affectedRows : 0;
        if (affected > 0) {
            console.log(`Activated ${affected} .com sites (were inactive). Will retry fetch.`);
            return true;
        }
        return false;
    }
